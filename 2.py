...
def NOTIFIED_READ_MESSAGE(op):
    try:
        ...
        if op.type == 17:
            ...
            cb = Message()
            cb.to = op.param1
            cb.text = cl.getContact(op.param2).displayName + "\nสวัดดีคับ\nยินดีต้อนรับคับ\nเชิญส่องกันดูก่อนได้นะคับ\nท้าวใจดี  น่ารัก  เลขาก็สวย.."
            cl.sendMessage(cb)

        if op.type == 15:
            if op.param2 in Bots:
                return
            cl.sendText(op.param1, "อ้าว  ออกทำ.....อะไรละ")
            print "MemberLeft"
...
def bot(op):
    try:
        ...
        if op.type == 26:
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
                ...
                cl.sendMessage(msg)
            elif msg.text in ["C6v1"]:
                msg.contentType = 13
                msg.contentMetadata = {'mid': Amid}
                ki.sendMessage(msg)
            elif msg.text in ["Cv52"]:
                msg.contentType = 13
                ...
                kk.sendMessage(msg)
                kc.sendMessage(msg)
            elif msg.text in ["can5cel","Can6cel"]:
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                ...
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Cv2 ourำl","Cv2 linน้ดk on"]:
                if msg.toType == 2:
            ...
            elif msg.text in ["Cv3 sdourl","Cv3 gglink on"]:
                if msg.toType == 2:
            ...
            elif msg.text in ["Cugdrl","Linkgff off"]:
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.preventJoinByTicket = True
                    cl.updateGroup(X)
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Cv1 cuggrl","Cv1 ligfnk off"]:
                if msg.toType == 2:
            ...
            elif msg.text in ["Cv2 hgcurl","Cv2 lihfnk off"]:
                if msg.toType == 2:
                    X = kk.getGroup(msg.to)
                    X.preventJoinByTicket = True
                    kk.updateGroup(X)
                    ...
                    if wait["lang"] == "JP":
                        ...
                        kk.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Cv3 chuurl","Cv3 linsfk off"]:
                if msg.toType == 2:
                    X = kc.getGroup(msg.to)
                    X.preventJoinByTicket = True
                    kc.updateGroup(X)
		...
		for ticket_id in n_links:
			if wait["atjointicket"] == True:
                ...
                if wait["autoAdd"] == False:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"è¦äº†å…³æ–­ã€‚")
            elif "Mefjssage change: " in msg.text:
                wait["message"] = msg.text.replace("Mescjsage change: ","")
            ...
            elif "Commenxt:" in msg.text:
                c = msg.text.replace("Commdent:","")
                if c in [""," ","\n",None]:
                    cl.sendText(msg.to,"message changed")
                else:
                    wait["comment"] = c
...
#-----------------------------------------------
            ...
            elif "Bajdjdhn @" in msg.text:
                if msg.toType == 2:
                    print "[Ban]ok"
                    _name = msg.text.replace("Bandjd @","")
            ...
            elif "Unbjdjan @" in msg.text:
                if msg.toType == 2:
                    print "[Unban]ok"
                    _name = msg.text.replace("Unbfnfhan @","")
                    _nametarget = _name.rstrip('  ')
                    gs = ki.getGroup(msg.to)
                    gs = kk.getGroup(msg.to)
                    gs = kc.getGroup(msg.to)
...
#-----------------------------------------------
            elif "Bc dbdh" in msg.text:
				bctxt = msg.text.replace("Bcdjdj ","")
				ki.sendText(msg.to,(bctxt))
				kk.sendText(msg.to,(bctxt))
				kc.sendText(msg.to,(bctxt))
#-----------------------------------------------

            elif msg.text in ["Cvgh say hi"]:
...
#-----------------------------------------------
            ...
            elif msg.text in ["Cv ggsay hinata pekok"]:
            ...
            elif msg.text in ["Cv gsay didik pekok"]:
            ...
            elif msg.text in ["Cv ggsay bobo ah","Bobo dulu ah"]:
...
#-----------------------------------------------
            elif msg.text in ["PIhNG","Pging","ping"]:
...
#------------------------------------------------------------------
                ...
                print "[Command]dp executed"			
#------------------------------------------------------------------
            elif msg.text in ["Bfnfhan"]:
                wait["wblacklist"] = True
                cl.sendText(msg.to,"send contact")
                ki.sendText(msg.to,"send contact")
                kk.sendText(msg.to,"send contact")
                kc.sendText(msg.to,"send contact")
            elif msg.text in ["Unbafjfjn"]:
                wait["dblacklist"] = True
                cl.sendText(msg.to,"send contact")
                ki.sendText(msg.to,"send contact")
                kk.sendText(msg.to,"send contact")
                kc.sendText(msg.to,"send contact")
            elif msg.text in ["Banfjfjlist"]:
                if wait["blacklist"] == {}:
...
#              if msg.from_ in admin:
		  if msg.to in wait2['readPoint']:
	            ...
	            cl.sendText(msg.to, "ไม่สามารถอ่านได้กรุณาตั้งค่าใหม่พิมพ์\n[อ่าน]\nตั้งค่าเสร็จสิ้นกรุณาพิมพ์\n[คนอ่าน]\nn[SELFBOT PHET MIN BOT]\n[By.มิน ทีมทดลองบอท]")

        if op.type == 17:
            group = cl.getGroup(op.param1)
            cb = Message()
            cb.to = op.param1
            cb.text = cl.getContact(op.param2).displayName + "🙏 ยินดีต้อนรับคับ 🙏\nเชิญส่องกันดูก่อนได้นะคับ\nท้าวใจดี  น่ารัก  เลขาก็สวย..💋"
            cl.sendMessage(cb)

        if op.type == 15:
            if op.param2 in Bots:
                return
            cl.sendText(op.param1, "โอกาสหน้ามาใหม่นะครับ .. ลาก่อน 👋")
            print "MemberLeft"
