...
targets = []
mid = cl.getProfile().mid
Bots = ["uf0bd4970771f26a8cef66473d59bcc69",mid]
self = ["uf0bd4970771f26a8cef66473d59bcc69",mid]
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     ...
                     balas = [cName + "\n" + str(wait["tag1"])
                     ret_ = "[🔯ตอบกลับ อัตโนมัติ🔯] " + random.choice(balas)
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     ...
                     cName = contact.displayName
                     balas = [cName + "\n" + str(wait["tag2"])
                     ret_ = "[🔯ออโต้🔯] " + random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
                     for mention in mentionees:
...
#=========================================
                ...
                ki10.sendMessage(msg)
#=======================================
            elif "ให้คุณ " in msg.text:
                       msg.contentType = 13
                       nk0 = msg.text.replace("ให้คุณ ","")
                       nk1 = nk0.lstrip()
                       nk2 = nk1.replace("@","")
                       nk3 = nk2.rstrip()
                       _name = nk3
                       ...
                       for target in targets:
                            try:
                               ...
                               ki10.sendText(msg.to, "คิกเกอร์ 10.👌\nแปลงร่าง อวตาล\nเสร็จเรียบร้อย (^_^)")
                                ...
                                print e
#====================================
            elif msg.text in ["คิก กลับร่าง"]:
                try:
                    ki1.updateDisplayPicture(backup1.pictureStatus)
                    ki1.updateProfile(backup1)
                    ...
                    ki6.updateDisplayPicture(backup5.pictureStatus)
                    ki6.updateProfile(backup6)

                    ki7.updateDisplayPicture(backup5.pictureStatus)
                    ki7.updateProfile(backup7)

                    ki8.updateDisplayPicture(backup5.pictureStatus)
                    ki8.updateProfile(backup8)

                    ki9.updateDisplayPicture(backup5.pictureStatus)
                    ki9.updateProfile(backup9)

                    ki10.updateDisplayPicture(backup5.pictureStatus)
                    ki10.updateProfile(backup10)

                    cl.sendText(msg.to, "คิกเกอร์ทั้งหมด👌\nกลับร่าง มหาเทพแล้ว (^_^)")
                except Exception as e:
                    cl.sendText(msg.to, str(e))
       ...
       #-------------Fungsi Respon Start---------------------#
        ...
        if op.type == 17:
            if wait["Notifed"] == True:
                ...
                cl.sendMessage(c)
                cl.sendImageWithUrl(op.param1,image)
                cl.sendText(op.param1,"[🙋สวัสดี จร้า🙋] " + cl.getContact(op.param2).displayName + "\n😈ยินดีตอนรับ😈 เข้าสู่ ☞ " + str(ginfo.name) + " ☜")
